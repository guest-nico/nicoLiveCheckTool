...
namespace namaichi.alart
{
	...
	public class AppPushReceiver
	{
		...
		public void start()
		{
			string id = config.get("appPushId");
			string token = config.get("appPushToken");
		...
		private bool getOkIdToken(out string id, out string token)
		{
			...
			if (!checkin(out string _id, out string _token))
			{
				return false;
			}

			string pushToken = getToken(_id, _token);
			if (pushToken == null)
			{
				return false;
			}

			if (!sendTokenNico2(pushToken))
			{
				return false;
			}
		...
		private bool checkin(out string id, out string token)
		{
			...
			try
			{
				string url = "http://android.clients.google.com/checkin";

				CheckinRequest cr = new CheckinRequest
				{

					//cr.Imei = "109269993813709";// # "".join(random.choice("0123456789") for _ in range(15))
					Imei = "000000000000000",// # "".join(random.choice("0123456789") for _ in range(15))
					androidId = 0
				};


				CheckinRequest.Checkin.Build build = new CheckinRequest.Checkin.Build
				{
				...
				CheckinRequest.Checkin checkinM = new CheckinRequest.Checkin
				{
					build = build
				};
				cr.checkin = checkinM;
				...
				*/
				...
				byte[] rb = util.postResBytes(url, headers, postDataBytes);
				if (rb == null)
				{
				...
				using (MemoryStream ms = new MemoryStream(rb))
				{
		...
		private string getToken(string androidId, string securityToken)
		{
			...
			try
			{
				...
				string url = "https://android.clients.google.com/c2dm/register3";
				string r = util.postResStr(url, headers, postDataBytes);
				util.debugWriteLine(r);
				string token = new Regex("token=(.+)").Match(r).Groups[1].Value;
		...
		private bool sendTokenNico(string pushToken)
		{
			...
			try
			{
				...
				string urlCookie = check.container.GetCookieHeader(new Uri("https://live2.nicovideo.jp")) + ";";
				string userSession = util.getRegGroup(urlCookie, "user_session=(.+?);");

				string url = "https://api.gadget.nicovideo.jp/notification/clientapp/registration";
				...
				string param = "token=" + userSession;
		...
		private bool sendTokenNico2(string pushToken)
		{
			...
			try
			{
				...
				Dictionary<string, string> acPassHeaders = new Dictionary<string, string>() {
					//{"Content-Type", "application/json; charset=UTF-8"},
				...
				string url = "https://api.cas.nicovideo.jp/v1/services/ex/app/nicocas_android/installations";

				string param = "{\"token\": \"" + pushToken + "\"}";
				byte[] postDataBytes = Encoding.ASCII.GetBytes(param);
				string res = util.postResStr(url, headers, postDataBytes);
				if (res == null)
				{
					check.form.addLogText("スマホ通知のトークンの送信に失敗しました");
				}
				...
				HttpWebResponse _res = util.sendRequest(url, headers, postDataBytes, "DELETE");
				if (res == null)
				{
					check.form.addLogText("スマホ通知のブロック設定の送信に失敗しました");
				}

				if (res != null)
				{
					using (Stream getResStream = _res.GetResponseStream())
					using (StreamReader resStream = new System.IO.StreamReader(getResStream))
					{
						string _r = resStream.ReadToEnd();
				...
				{
					util.debugWriteLine("app push blocks delete null");
				}
				...
				if (res == null)
				{
					check.form.addLogText("スマホ通知の時間設定の送信に失敗しました");
				}
				...
				{
					util.debugWriteLine("app push time put null");
				}
		...
		public bool connectMcs(string androidId, string securityToken)
		{
			...
			try
			{
				LoginRequest lr = new LoginRequest
				{
					...
					DeviceId = "android-" + long.Parse(androidId).ToString("x"),
					NetworkType = 1,
					Resource = androidId,
					User = androidId,
					UseRmq2 = true,
					AccountId = long.Parse(androidId)
				};
				...
				using (TcpClient client = new TcpClient("mtalk.google.com", 5228))
				using (sslStream = new SslStream(client.GetStream(), false, delegate { return true; }))
				{
					...
					byte[] _buf = VarintBitConverter.GetVarintBytes((uint)x.Length);
					/*
				    var num = x.Length;
					...
					*/
					...
					int version = sslStream.ReadByte();
					...
					while (isRetry)
					{
						int responseTag = sslStream.ReadByte();
								...
								|| responseTag == 0x08)
						{
							...
							int length = 0;
							List<byte> _lenBuf = new List<byte>();
							for (int i = 0; i < 10; i++)
							{
								int _slb = sslStream.ReadByte();
								...
								try
								{
									...
									int length2 = VarintBitConverter.ToInt16(_lenBuf.ToArray()) * 1;
									long length3 = VarintBitConverter.ToInt64(_lenBuf.ToArray()) * 1;
									int length4 = VarintBitConverter.ToUInt16(_lenBuf.ToArray()) * 1;
									uint length5 = VarintBitConverter.ToUInt32(_lenBuf.ToArray()) * 1;
									length = (int)length5;
									break;
								}
								catch (Exception e)
							...
							while (msg.Count < length)
							{
								...
								int i = sslStream.Read(readbuf, 0, length - msg.Count);
								for (int j = 0; j < i; j++)
								{
									msg.Add(readbuf[j]);
								}
						...
						if (responseTag == 0x03)
						{
							...
							using (MemoryStream ms = new MemoryStream(msg.ToArray()))
							{
						...
						else if (responseTag == 0x08)
						{
							try
							{
								...
								string d = "";
								foreach (AppData a in lresp.AppDatas)
								{
									d += a.Value;
								}
								//var lvid = util.getRegGroup(d, "program_id\\\\\":\\\\\"(lv\\d+)"); google protobuf
								string lvid = util.getRegGroup(d, "program_id\":\"(lv\\d+)"); //google protobuf 
								if (lvid != null)
								{
									util.debugWriteLine("appData lvid " + lvid);
									List<RssItem> items = getNicoCasItem(lvid, lresp);
									if (items != null)
									{
									...
									{
										GetItemRetryApr gir = new GetItemRetryApr(lvid, lresp, this);
										Task.Factory.StartNew(() =>
										{
											for (int i = 0; i < 10; i++)
											{
		...
		public List<RssItem> getItem(string lvid, McsProto.DataMessageStanza msg)
		{
			try
			{
				...
				rec.HosoInfoGetter hg = new namaichi.rec.HosoInfoGetter();
				bool r = hg.get(lvid, check.container);
				...
				if (!r)
				{
					...
					hg.tags = new string[] { };
					Regex reg = new Regex("\\[生放送開始\\](.+?)さんが「(.+?)」を開始しました。");
					Match m = reg.Match(msg.AppDatas.ToString());
					if (m.Length != 0)
					{
				...
				else
				{
					if (hg.type == "community" || hg.type == "user")
					{
						Regex reg = new Regex("\\[生放送開始\\](.+?)さんが「(.+?)」を開始しました。");
						string d = "";
						foreach (AppData a in msg.AppDatas)
						{
							d += a.Value;
						}
					...
					{
						Regex reg = new Regex("\\[生放送開始\\](.+?)「(.+?)」を開始しました。");
						string d = "";
						foreach (AppData a in msg.AppDatas)
						{
							d += a.Value;
				...
				RssItem i = new RssItem(title, lvid, dt.ToString(), hg.description, comName, hg.communityId, hostName, hg.thumbnail, hg.isMemberOnly.ToString(), "");
	...
	internal class GetItemRetryApr
	{
		private readonly string lvid;
		private readonly McsProto.DataMessageStanza lresp;
		private readonly AppPushReceiver pr;
